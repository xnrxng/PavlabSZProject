...
### create a master list
cohort_list <- c("CMC", "SZBDMulti-Seq", "Batiuk", "MultiomeBrain")
...
for (cell_type in cell_types) {
  ...
  for (cohort in cohort_list) {
    ...
    if (!file.exists(file_path)) {
      message("File ", file_path, " does not exist. Skipping to next.")
      next
    }
...
saveRDS(DGE, "results/DEA/AllMetaanalysis/DGE_meta.rds")
